...
namespace Kudos.Models.bases {
	public abstract class SettingBase : INotifyPropertyChanged {
		...
		protected SettingBase(SettingNames name) => Name = name;
		...
		public static Setting<T> Create<T>(SettingNames name, T defaultValue) => new Setting<T>(name, defaultValue);
		...
		public static DictionarySetting<T1, T2> Create<T1, T2>(SettingNames name, ImmutableDictionary<T1, T2> defaultValue) =>
			new DictionarySetting<T1, T2>(name, defaultValue);
